...
void error(libusb_device_handle* device, int interface,char *message, int error_code, int err_no)
{
    printf("%s (%s)(%s)\n",message, libusb_strerror(error_code), strerror(err_no));
    end_usb(device, interface);
    exit(error_code);    
}

libusb_device_handle* init_usb(uint16_t vendor_id, uint16_t product_id, int interface)
{
    int r;
    ...
    if (r < 0){
        error(device,interface,"Error claim interfase",r,errno);
    }
    
    return device;
}

void change_color(libusb_device_handle* device, int interface, unsigned char cyan,unsigned char magenta, unsigned char yellow)
{
...
//    r = libusb_control_transfer (device, 0x21, 9, 0x0302, 2, data, 16, 0);
    ...
    if (r < 0){
        error(device,interface,"Error in transfer",r,errno);
    }
...
int main()
{
    ...
    device = init_usb(vendor_id, product_id, interface);
    
    //change_color(device,0xf0,0xf0,0x42); //blue
    change_color(device,interface,0xff,0xff,0x00); 
